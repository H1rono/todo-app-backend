...
impl Database {
    pub async fn fetch_all_todos(&self) -> Result<Vec<Todo>> {
        sqlx::query("SELECT `id`, `title`, `note`, `due_to`, `created_at`, `done`, `created_at`, `updated_at`, `deleted_at` FROM `todos`")
            .fetch_all(&self.pool)
            .await
    ...
    pub async fn fetch_todo_by_id(&self, id: u32) -> Result<Todo> {
        sqlx::query("SELECT `id`, `title`, `note`, `due_to`, `created_at`, `done`, `created_at`, `updated_at`, `deleted_at` FROM `todos` WHERE `id` = ? LIMIT 1")
            .bind(id)
            .fetch_all(&self.pool)
            .await
    ...
    pub async fn fetch_todos_like_title(&self, title: &str) -> Result<Vec<Todo>> {
        sqlx::query("SELECT `id`, `title`, `note`, `due_to`, `created_at`, `done`, `created_at`, `updated_at`, `deleted_at` FROM `todos` WHERE `title` LIKE ?")
            .bind(title)
            .fetch_all(&self.pool)
            .await
    ...
    pub async fn fetch_todos_by_title(&self, title: &str) -> Result<Vec<Todo>> {
        sqlx::query("SELECT `id`, `title`, `note`, `due_to`, `created_at`, `done`, `created_at`, `updated_at`, `deleted_at` FROM `todos` WHERE `title` = ?")
            .bind(title)
            .fetch_all(&self.pool)
            .await
    ...
    pub async fn fetch_todos_like_note(&self, note: &str) -> Result<Vec<Todo>> {
        sqlx::query("SELECT `id`, `title`, `note`, `due_to`, `created_at`, `done`, `created_at`, `updated_at`, `deleted_at` FROM `todos` WHERE `note` LIKE ?")
            .bind(note)
            .fetch_all(&self.pool)
            .await
    ...
    pub async fn fetch_todos_by_note(&self, note: &str) -> Result<Vec<Todo>> {
        sqlx::query("SELECT `id`, `title`, `note`, `due_to`, `created_at`, `done`, `created_at`, `updated_at`, `deleted_at` FROM `todos` WHERE `note` = ?")
            .bind(note)
            .fetch_all(&self.pool)
            .await
    ...
    pub async fn fetch_todos_by_done(&self, done: bool) -> Result<Vec<Todo>> {
        sqlx::query("SELECT `id`, `title`, `note`, `due_to`, `created_at`, `done`, `created_at`, `updated_at`, `deleted_at` FROM `todos` WHERE `done` = ?")
            .bind(done as i8)
            .fetch_all(&self.pool)
            .await
